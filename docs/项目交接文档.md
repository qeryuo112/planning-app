...
## 13. 2026-08-16 FCM 代理端到端验证

### 完成内容

1. 在服务器 `/opt/mihomo` 部署 mihomo v1.18.8，systemd 运行，监听 `127.0.0.1:7890`。
2. 通过订阅转换脚本生成 Clash YAML，仅 Google/FCM 域名走代理，默认节点 `德国法兰克福-rl(主力)`。
3. 修正 `FcmService`：原来只设置 `HTTPS_PROXY`/`HTTP_PROXY` 对 `firebase-admin` 内置 HTTP 客户端无效，改为使用 `https-proxy-agent` 创建 `HttpsProxyAgent`，并传入 `cert(credential, proxyAgent)` 和 `initializeApp({ credential, httpAgent: proxyAgent })`。
4. 在 `services/api/package.json` 新增依赖 `https-proxy-agent: ^7.0.6`。
5. 本地构建 `services/api` 后上传 `dist/` 到服务器，重启 `planning-api.service`。
6. 使用测试账号创建 `channel=push` 提醒，15 秒扫描任务触发后，mihomo 日志成功显示 `[TCP] ... --> fcm.googleapis.com:443 ... using PROXY[德国法兰克福-rl(主力)]`，API 日志无 FCM 失败，说明服务端已成功投递到 FCM。
...
curl -x http://127.0.0.1:7890 https://fcm.googleapis.com/fcm/send -o /dev/null -w "%{http_code}\n"
...
- `planning-api.service`：active (running)
- `mihomo.service`：active (running)
- `/opt/planning-app/.env`：`GOOGLE_API_PROXY=http://127.0.0.1:7890`
- 测试账号：`planning-test@example.com` / `Test@123456`
- 真机是否收到通知取决于 `users.fcmToken` 是否为当前登录设备的有效 token。

### 遗留

- 代理节点长期稳定性需观察，订阅失效或节点异常时切换节点/订阅。
- 如需确认真机送达，建议在当前登录设备上重新上传 FCM token 后再次测试。
